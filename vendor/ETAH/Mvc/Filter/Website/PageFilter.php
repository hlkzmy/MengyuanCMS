...
class PageFilter extends BaseFilter
{
	...
	public function getSubInputFilter()
	{
		...
		$nameStringLength->setMax(100);
		$nameStringLength->setMessage('页面名称不能超过 %max% 个字符',$stringLength::TOO_LONG);
		$nameStringLength->setMessage('页面名称不能小于 %min% 个字符',$stringLength::TOO_SHORT);
		$nameStringLength->setMessage('asdas',$stringLength::INVALID);
		
		$nameEmpty = clone $empty;
		$nameEmpty->setMessage('页面名称不能为空');
		...
		$namespaceStringLength->setMax(100);
		...
		$controllerStringLength->setMax(100);
