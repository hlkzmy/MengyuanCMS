...
namespace Etah\Mvc\Plugin\Permission;
...
class Permission extends AbstractPlugin
{
	...
	public function menu($e){
		...
		$menuModel = $serviceManager->get('Base/Plugin/Permission/Model/MenuModel');
		
// 		$menuModel->rebuildStructureTree(1,1,-1);
	...
	{	
		...
		if($this->user==null){
			...
			$userLoginUrl = $router->assemble(array(), array('name'=>'admin'));
		...
		//1.得到相关的模型
		$userRoleModel = $serviceManager->get('Base/Plugin/Permission/Model/UserRoleModel');
		$accessModel   = $serviceManager->get('Base/Plugin/Permission/Model/AccessModel');
		$nodeModel     = $serviceManager->get('Base/Plugin/Permission/Model/NodeModel');
