...
class Content{
	...
	private function getDataByParseContentConfig($controller,$contentConfig){
		...
		//1.查询对应文章的id文章列表
		$article_ids = $contentConfig['article_ids'];
		...
		if (sizeof($article_ids)> 0){
			$articleList = $controller->articleModel->getRowById($article_ids,$articleListColumns);
		}
		...
		//2.获取组件标题
		$component_title = $contentConfig['component_title'];
		...
		$data['component_title'] = $component_title;
	...
	public function render($controller,$contentConfig){
		...
		$viewModel->setTemplate('component/Article/common2/template');
