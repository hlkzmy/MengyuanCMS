...
class Content{
	...
	private function getDataByParseContentConfig($controller,$contentConfig){
		...
		$data['video_sort_url']  = $controller->url()->fromRoute('resource',array('controller'=>'videoSort','action'=>'list','id'=>$videoSortId));
																					
		$data['video_sort_name'] = $videoSortName;
		$data['list'] 			 = array_chunk($videoList, $contentConfig['per_row_count']);
	...
	public function render($controller,$contentConfig){
		...
		$viewModel->setTemplate('Component/Video/Listing/Common/template');
