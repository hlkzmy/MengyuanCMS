...
class Content{
	...
	private function getDataByParseContentConfig($controller,$contentConfig){
		...
		if (sizeof($descendantVideoSortIdList) > 0 ){
			...
			$videoList = $controller->videoModel->getRowByCondition($where,array('id','name','thumb'),$videoLimit);
		}
		...
		$data['videoList'] 			 	= array_chunk($videoList, $contentConfig['per_row_count']);
	...
	public function render($controller,$contentConfig){
		...
		$viewModel->setTemplate('Component/Video/Resource/tab/template');
