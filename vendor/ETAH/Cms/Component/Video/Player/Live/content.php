...
namespace Etah\Cms\Component\Video\Player\Live;
...
class Content extends EtahMvcBaseController{
	 ...
	 * @param bool $bool
	 */
	public function setMultipleRecord($bool)
	{
		$this->multipleRecord = $bool;
	...
	public function render(){
		...
		try{
			...
			if (isset($this->multipleRecord) && $this->multipleRecord ===true){
				
				foreach($this->streamList as $streamList){
					
					$server_ip = $streamList['server_ip'];
					
					foreach($streamList['stream'] as $stream){
						...
						$rtmp->setSourceServerParameter($server_ip,'teach_app','123',$sourceStreamName,'1111',1935);
		...
		if(in_array($operationSystemName,array('windows','linux','android'))){
			...
			$viewModel->setTemplate('Component/Video/Player/Live/Template/flash-player');
		}
		...
		//if(true){
			...
			$viewModel->setTemplate('Component/Video/Player/Live/Template/html5-player');
		}
		else{
