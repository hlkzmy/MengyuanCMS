...
class Content extends EtahMvcBaseController{
	...
	public function controllerRender(){
		...
		if($operationSystemName=='unknown'){
			...
			$viewModel->setTemplate('Component/Video/Player/NewRecord/Template/flash');
		}
			...
			$viewModel->setTemplate('Component/Video/Player/NewRecord/Template/html5');
		}
	...
	public function render($controller,$contentConfig){
		...
		$phpRenderer->headScript()->appendFile($basePath.'/js/org/jquery/jquery.raty.js');
...
// 		$local_server_ip = $websiteConfig['local_server_ip'];
		
		$recordIP = sprintf("%s:%s",$_SERVER['HTTP_HOST'],$websiteConfig['record_video_play_parameter']['fms_port']);
		...
		if ($video['record_type'] == 'r'){
			...
			$videoPlayInfo = $this->videoPlayinfoModel->getRowById($id);
			if (sizeof($videoPlayInfo) < 1  ){
				$this->videoPlayinfoModel->insertRow(array(
						'id'=>$id,
						'chapterNumber'=>$chapterNumber,
						'length'=>$videoFileList[0]['time']
				));
			}elseif(isset($videoPlayInfo['length']) && $videoPlayInfo['length'] == 0){
				$this->videoPlayinfoModel->updateRowById($id,array(
						'length'=>$videoFileList[0]['time']
				));
			}elseif(isset($videoPlayInfo[0]['length']) && $videoPlayInfo[0]['length'] == 0){
				$this->videoPlayinfoModel->updateRowById($id,array(
						'length'=>$videoFileList[0]['time']
				));
			}
			...
			foreach($videoFileList as $key=>$videoFile){
				...
				if($videoFile['length'] > $videoLength) {$videoLength = $videoFile['length']; }
			
			}//foreach end
		}
			
		//更新视频点击数
		$this->videoModel->updateRowById($id,array('pv'=>$video['pv']+1));
		...
		//如果是对知识点的播放，那么重新计算剪辑数据
		if(isset($postData['point']) && $postData['point'] != null ){
			$cutinfo['time'] = array();
			$point = explode('-', $postData['point']);
			if(sizeof($point) == 1){
				//只有一个点表示从这个点播到结尾,意思是剪辑这个点之前的内容
				$cutinfo['time'][0] = array('start'=>0,'end'=>(int)$point[0]);
				
			}elseif (sizeof($point) == 2){
				//有两个点的时候，减去除这两个点之外的所有
				$cutinfo['time'][0] = array('start'=>0,'end'=>(int)$point[0]);
				$cutinfo['time'][1] = array('start'=>(int)$point[1],'end'=>(int)$videoLength);
			}
		...
		$RecordVideoPlayInfo = array(
				...
				'default'=>false,
				'ip'=> sprintf("%s:%s",$websiteConfig['local_server_ip'],$websiteConfig['record_video_play_parameter']['fms_port']),
				'video'=>$videoFileList,
		);
	...
	public function ajaxSetVideoSorceAction()
	{
		$request = $this->getRequest();
		...
		if(isset($postData['score'])  && isset($postData['videoId'])){
			try{
				$videoColumns = array('id','pv','score');
				$video = $this->videoModel->getRowById($postData['videoId'],$videoColumns);
				$score = ($video['score']*$video['pv']+$postData['score'])/($video['pv']+1);
				$this->videoModel->updateRowById($postData['videoId'],array('score'=>$score>10?10:$score));
				
			}catch ( \Exception $e){
				$this->ajaxReturn('300', $e->getMessage());
			}
		}else{
			$this->ajaxReturn('300', '没有获取到视频ID或分数');
		}
		
		$this->ajaxReturn('200', $score);
