...
class Content{
	...
	private function getDataByParseContentConfig($controller,$contentConfig){
		...
		$coursewareColumns = array('id','name','add_user_id','courseware_sort_id','video_id','description','update_time');
		
		$courseware = $controller->coursewareModel->getRowById($coursewareId,$coursewareColumns);
		
		
		//2.根据课件类别或者根据课件所属的视频查询课件列表
		
		$coursewareSortId = $courseware['courseware_sort_id'];
		
		$videoId = $courseware['video_id'];
		...
		switch ($type){
			case 'bysort':{
				...
				$where->notEqualTo('id', $coursewareId);
				break;
			}
			case 'byvideo':{
			...
			case 'all':{
				...
				$cwhere = new Where();
				$cwhere->equalTo('courseware_sort_id', $coursewareSortId);
				$cwhere->or;
				$cwhere->equalTo('video_id', $videoId);
				$where->addPredicate($cwhere);
				$where->notEqualTo('id', $coursewareId);
				break;
			}
		}
	...
	public function render($controller,$contentConfig){
		...
		$viewModel->setTemplate('component/Courseware/OtherCourseware/template');
